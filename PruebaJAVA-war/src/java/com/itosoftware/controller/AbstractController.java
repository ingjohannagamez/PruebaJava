...
import com.ito.prueba.persistencia.dao.GenericDAO;
...
public abstract class AbstractController<T> implements Serializable {
    ...
    @EJB
    private GenericDAO<T> ejbFacade;
     ...
     * bean.
     */
    protected GenericDAO<T> getFacade() {
        return ejbFacade;
    }
    ...
    public List<T> getItems() {
        if (items == null) {
            items = this.ejbFacade.buscarTodo();
        }
        return items;
    }
        ...
        if (selected != null) {
            ...
            try {
                if (persistAction != PersistAction.DELETE) {
                    this.ejbFacade.actualizar(selected);
                } else {
                    this.ejbFacade.eliminar(selected);
                }
    ...
    private List<T> refreshItem(T item, List<T> items) {
